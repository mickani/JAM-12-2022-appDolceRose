...
if(isset($_POST['id_compra']) && isset($_POST['id_materia_prima'])) {
  ...
  $cantidad = $data[0]; //Obtengo la cantidad


  // Elimino detalle
  $query = "DELETE FROM detalles_compras WHERE id_compra = $id_compra AND id_materia_prima = $id_materia_prima"; 
  $result = mysqli_query($conn, $query);
  if(!$result) {
    die("Query Failed.");
  }
